...
export function InsightsLayout() {
  ...
  const subscribe = (event) => {
    ...
    saveSubmission('newsletter', data);
    setSubscribed(true);
  };
  ...
  return (
    <main className="overflow-hidden bg-white text-[#091E42]">
      ...
      <section id="contact-insights" className="bg-[#091E42] py-20 text-white sm:py-24">
        <div className="mx-auto grid max-w-[1200px] gap-10 px-4 sm:px-6 lg:grid-cols-[.9fr_1.1fr] lg:items-center lg:px-8">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[.2em] text-[#8DCBE9]">Enterprise intelligence, delivered</p>
            <h2 className="mt-4 text-3xl font-semibold tracking-[-.045em] sm:text-4xl">Stay ahead of recovery trends.</h2>
            <p className="mt-5 max-w-lg leading-7 text-slate-300">Receive focused recovery insights, industry intelligence and compliance updates. When you need tailored guidance, our specialists are ready to help.</p>
            <Link to="/contact" className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-[#8DCBE9]">Talk to an expert <ArrowRight className="h-4 w-4" /></Link>
          </div>
          <form onSubmit={subscribe} className="rounded-3xl border border-white/15 bg-white/[.06] p-6 backdrop-blur-sm sm:p-8">
            <div className="grid gap-4 sm:grid-cols-2">
              <label htmlFor="newsletter-name" className="text-xs font-semibold text-white">Name
                <input id="newsletter-name" required name="name" className="mt-2 w-full rounded-xl border border-white/15 bg-white/10 px-3.5 py-3 text-sm text-white outline-none placeholder:text-slate-400 focus:border-[#8DCBE9]" />
              </label>
              <label htmlFor="newsletter-company" className="text-xs font-semibold text-white">Company
                <input id="newsletter-company" required name="company" className="mt-2 w-full rounded-xl border border-white/15 bg-white/10 px-3.5 py-3 text-sm text-white outline-none focus:border-[#8DCBE9]" />
              </label>
            </div>
            <label htmlFor="newsletter-email" className="mt-4 block text-xs font-semibold text-white">Business email
              <input id="newsletter-email" required type="email" name="email" className="mt-2 w-full rounded-xl border border-white/15 bg-white/10 px-3.5 py-3 text-sm text-white outline-none focus:border-[#8DCBE9]" />
            </label>
            <button type="submit" className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-white px-5 py-3.5 text-sm font-semibold text-[#091E42] transition hover:bg-[#DDF2FC]">
              <Mail className="h-4 w-4" /> {subscribed ? '✓ Subscribed & Saved' : 'Subscribe to insights'}
            </button>
          </form>
        </div>
      </section>
