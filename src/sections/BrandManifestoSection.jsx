 ...
 * ─────────────────────────────────────────────────────────────────────
 * A centered, text-only scroll moment — three short statements sit on
 * screen together and light up word-by-word, in order, as the visitor
 * scrolls through this section — the same "read as you scroll" sweep built
 * for the Services page story sections, but a single continuous pass (no
 * chapters, no images) since this is one short passage, not a multi-part
 * story. Used on the homepage (between the trust badges and the services
 * overview) and on the Services page (after the mandate cards).
 *
...
function StaticManifesto({ lines }) {
  return (
    <motion.div
      variants={stagger}
      initial="hidden"
...
export default function BrandManifestoSection({ lines: linesProp }) {
  const lines = linesProp ?? DEFAULT_LINES;
  ...
  return (
    <section className="relative z-30 bg-white" aria-label="Our operating principles">
      ...
      <ManifestoStage wrapRef={wrapRef} wordRefs={wordRefs} lines={lines} />
    </section>
  );
}
...
function ManifestoStage({ wrapRef, wordRefs, lines }) {
  let wordCounter = 0;
  return (
